    ...
    <Box className="bg-white rounded-xl w-full px-2">
      <Box className="flex w-full justify-around gap-3 my-5 rounded-2xl p-2 bg-[#edeef09c] text-center">
        <Box
          ...
          className={`${userSelection === "scoringPlays" && "button-active"} flex-grow h-full text-xs font-semibold`}
        >
          Scoring Plays
        </Box>
        <Box
          onClick={() => setUserSelection("allPlays")}
          className={`${userSelection === "allPlays" && "button-active"} flex-grow h-full text-xs font-semibold`}
        >
          All Plays
        </Box>
      </Box>
