...
export default function NFLScoringPlays({ data }: { data: any }) {
  function scoringPlays(allScoringPlays: any) {
    const plays = allScoringPlays.map((play: any) => {
      return (
        <Box key={uuidv4()} className="w-full flex flex-row justify-between gap-2 items-center mb-2">
          <Box className="flex flex-row items-center gap-2">
            <Image src={play.team.logo} width={100} height={100} alt="team logo" className="w-8 object-contain" />
          ...
          <Box className="flex flex-row gap-6 pr-2">
            <Typography
              sx={{
                fontWeight: play.team.displayName === data.homeTeam.team.displayName ? "700" : "400",
              }}
            ...
            <Typography
              sx={{
                fontWeight: play.team.displayName === data.awayTeam.team.displayName ? "700" : "400",
              }}
              className="w-4 text-center"
            >
              {play.awayScore}
